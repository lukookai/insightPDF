...
class PPDocLayoutPostProcess:
    ...
    def __call__(
        ...
        """
        支持 batch：
          - 当 len(batch_outputs)==len(datas)==N>1 时，返回 batch 结果（三个二维list）
          - 当 N==1 时，为兼容旧逻辑，返回单图结果（三个一维list）
        """
        if datas is None or batch_outputs is None:
            raise ValueError("datas or batch_outputs is None")
        if len(datas) != len(batch_outputs):
            raise ValueError(f"len(datas)={len(datas)} != len(batch_outputs)={len(batch_outputs)}")

        outputs_per_image: List[Boxes] = []

        for idx, (data, output) in enumerate(zip(datas, batch_outputs)):
            # 每张图独立决定 shape_mode，避免污染后续样本
            shape_mode_i = layout_shape_mode

            if "masks" in output and output["masks"] is not None:
                masks = output["masks"]
            else:
                masks = None
                # 没 masks 的模型只能输出 rect；auto 也只能退化成 rect
                # 注意：这里不要修改外部的 layout_shape_mode 变量，改局部 shape_mode_i
                shape_mode_i = "rect"
                if idx == 0 and (layout_shape_mode not in [None, "rect", "auto"]):
                    print(
                        f"The model you are using does not support polygon output, "
                        f"but layout_shape_mode={layout_shape_mode}, will fallback to 'rect'."
                    )
            ...
            if filter_overlap_boxes:
                boxes = filter_boxes(boxes, shape_mode_i)

            skip_order_labels_i = skip_order_labels if skip_order_labels is not None else SKIP_ORDER_LABELS
            boxes = update_order_index(boxes, skip_order_labels_i)

            outputs_per_image.append(boxes)

        # ---- 组装返回 ----
        boxes_batch: List[List[Any]] = []
        scores_batch: List[List[float]] = []
        class_names_batch: List[List[str]] = []

        for output in outputs_per_image:
            b, s, n = [], [], []
            for item in output:
                b.append(item["coordinate"])
                s.append(float(item["score"]))
                n.append(item["label"])
            boxes_batch.append(b)
            scores_batch.append(s)
            class_names_batch.append(n)

        # 兼容旧接口：batch=1 仍返回一维 list
        # if len(boxes_batch) == 1:
        #     return boxes_batch[0], scores_batch[0], class_names_batch[0]

        return boxes_batch, scores_batch, class_names_batch
