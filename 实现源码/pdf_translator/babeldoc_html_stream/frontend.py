...
def skill_translation_bridge(
    *,
    ...
    except Exception as exc:  # double6 Skill 兼容层未随迁移包提供时回退
        LOGGER.warning(
            "未找到 double6 翻译兼容层（%s/scripts/translation_compat_proxy.py），"
            "回退到本地 lawai 桥接直接调用 DeepSeek：%s",
            skill_root,
            exc,
        )

    if proxy_module is not None:
        ...
        # 回退：BabelDOC 直接对接本地 lawai OpenAI 兼容端点
        try:
            yield f"http://127.0.0.1:{lawai_port}/v1", {}
        finally:
            lawai_server.shutdown()
            lawai_server.server_close()
            lawai_thread.join(timeout=2)
